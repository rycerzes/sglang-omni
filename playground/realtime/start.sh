...
# ---------------------------------------------------------------------------
# Start the realtime voice chat playground.
#
# Usage:
#   ./playground/realtime/start.sh --model-path Qwen/Qwen3-Omni-30B-A3B-Instruct
#   CUDA_VISIBLE_DEVICES=5 ./playground/realtime/start.sh --model-path <path>
#   ./playground/realtime/start.sh --model-path <path> --port 8000 --playground-port 7861
# ---------------------------------------------------------------------------
...
# Parse arguments: extract our flags, forward the rest to sglang-omni serve
BACKEND_ARGS=()
while [[ $# -gt 0 ]]; do
  case "$1" in
    --port)             PORT="$2"; shift 2 ;;
...
echo "============================================================"
echo "  SGLang-Omni Realtime Voice Chat"
echo "============================================================"
echo ""
echo "  Backend API:   http://localhost:${PORT}"
echo "  Realtime UI:   http://localhost:${PLAYGROUND_PORT}"
echo "  WebSocket:     ws://localhost:${PORT}/v1/realtime"
echo ""
echo "============================================================"
echo ""
...
# 3. Launch the playground UI (foreground — Ctrl-C stops everything via trap)
